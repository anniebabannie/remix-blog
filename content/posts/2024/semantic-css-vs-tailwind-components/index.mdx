...
So, while both semantic CSS and Tailwind abstract away details, Tailwind does it better. It is always quicker to write the Tailwind class than the underlying CSS. For example, let's say you're building a grid of elements with three columns. In Tailwind, you could use the class <pre>grid-cols-3</pre> , which is much shorter than its definition: <pre>grid-template-columns: repeat(3, minmax(0, 1fr));</pre>
